...
- **`proxy/`** — the glue and the Inertia mirror:
  ...
  - `mirror_control` re-sends the connected validator's pubkey as a `SOURCE_CONTROL` datagram every `CONTROL_INTERVAL` (1s). The server needs it to know whose leader schedule to apply before it will emit dangerous frontruns, and it **fails closed** — if this stops, the server stops that MEV. It is deliberately a periodic re-send rather than send-on-change, so a lost datagram or a server restart self-heals within a second.
