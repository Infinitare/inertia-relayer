...
- **`relayer/`** — the relayer gRPC surface the validator talks to:
  ...
  - `forwarder.rs` (`Forwarder` / `ForwarderService`): implements the `Relayer` gRPC service. **Max 1 active validator subscription.** A single `run_event_loop` (crossbeam `select!`) fans verified packets out to subscribers in batches of `VALIDATOR_PACKET_BATCH_SIZE` (64), emits heartbeats every 100ms, and drops subscribers whose channels close. `get_tpu_configs` advertises `quic_port - 6` (the UDP TPU port convention). `packet_subscriptions` is the single source of truth for **which validator is connected** — that pubkey is authenticated, not asserted: it comes from the ed25519 challenge-response via the auth interceptor. `ConnectedValidator` is a read-only handle onto it, reached via `Relayer::connected_validator()` and consumed by the mirror. `get()` deliberately returns `Some` **only when exactly one validator is subscribed** — the max-1 cap makes that always true today, but if the cap is ever raised it fails closed rather than returning an arbitrary `HashMap` key, which would otherwise have the Inertia server silently apply the wrong validator's leader schedule.
...
- **`proxy/`** — the glue and the Inertia mirror:
  ...
  - `mirror_control` re-sends the connected validator's pubkey as a `SOURCE_CONTROL` datagram every `CONTROL_INTERVAL` (1s). The server needs it to know whose leader schedule to apply before it will emit dangerous frontruns, and it **fails closed** — the server holds the identity on a 3s lease (`Client::VALIDATOR_LEASE`), so if this stops, the lease lapses and that MEV stops. The lease is what makes the periodic re-send load-bearing rather than merely redundant: it doubles as the liveness signal, so `get()` returning `None` (validator unsubscribed) reaches the server as an absence within 3s. Do not "optimise" this into send-on-change — the server would pin the departed identity's leader schedule indefinitely, since the mirror's QUIC keep-alive means the connection never idles out to signal the departure. A lost datagram or a server restart self-heals within a second.
